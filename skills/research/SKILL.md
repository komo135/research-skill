...
When this skill says "subagent", it means a fresh agent analysis in a separate context for exactly two roles: Plan review before execution and Result analysis after execution. It is an agent-protocol role, not a dependency on any host's specific Task tool.
...
| Research ideation | `references/ideation.md` | When asked for research ideas, research directions, hypothesis candidates, or "what should we try next" before Prior-work grounding; use substrate ids, generation operators, anti-vacuity gate, evaluator feedback, and an anchor-stripped seed brief if anchors are already visible |
